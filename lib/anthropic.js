...
export function mapAnthropicError(err) {
  if (err?.code === "missing_api_key") {
    return { status: 500, message: "Configuration serveur invalide (cle IA manquante)." };
  }
  if (err instanceof Anthropic.AuthenticationError) {
    return { status: 500, message: "Configuration serveur invalide (cle IA refusee)." };
  }
  if (err instanceof Anthropic.RateLimitError) {
    return { status: 429, message: "Service IA sature, reessaie dans un instant." };
  }
  if (err instanceof Anthropic.BadRequestError) {
    return { status: 400, message: `Requete IA invalide : ${err.message}` };
  }
  if (err instanceof Anthropic.APIError) {
    return { status: 502, message: "Service IA indisponible, reessaie plus tard." };
  }
  return null;
}
