...
//     labeled.
//
// `npm_downloads` (see lib/npm-downloads.js) is a THIRD, unrelated field on
// the returned object: npm package download counts, an adoption signal —
// never derived from or mixed into totals/endpoints above.
import { readFile } from "node:fs/promises";
import { join } from "node:path";
import { cached } from "./cache.js";
import config from "../config.js";
import { computeNpmDownloads } from "./npm-downloads.js";
...
async function computeStatsUncached() {
  const [sondages, paiements, npmDownloads] = await Promise.all([
    readJsonl(sondagesFile),
    readJsonl(paiementsFile),
    computeNpmDownloads(),
  ]);
  ...
  return {
    ...
    // npm download counts for the 2 MCP packages this project publishes —
    // adoption signal, NEVER a revenue figure (see lib/npm-downloads.js).
    // Kept as its own top-level field, entirely separate from `totals` and
    // `endpoints` above, so it can never be mistaken for or mixed into the
    // payment numbers.
