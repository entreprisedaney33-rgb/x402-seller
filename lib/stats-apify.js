...
// In-flight de-duplication (2026-09-04, added alongside GET /stats/apify/public):
// without this, two callers landing at the exact moment the 15-minute cache just
// expired (e.g. the private admin route and the new public route hit at once)
// would each independently pass the `cacheAgeMs < CACHE_TTL_MS` check as false and
// call fetchLiveApifyRevenue() in parallel -> 2 real Apify API calls instead of 1.
// Both routes call the SAME exported computeApifyStats() below, so sharing one
// in-flight promise here covers both, whatever their number or origin.
let pendingComputation = null;

export async function computeApifyStats() {
  if (pendingComputation) return pendingComputation;
  pendingComputation = computeApifyStatsUncached().finally(() => {
    pendingComputation = null;
  });
  return pendingComputation;
}

async function computeApifyStatsUncached() {
...
// GET /stats/apify/public (2026-09-04) — scrubbed, key-less mirror of the object
// above, meant to be fetched directly from a static site's client-side JS (the
// Jarvis PWA) that structurally cannot hold STATS_KEY. Same arbitration already
// applied to GET /stats vs /stats/daily: aggregates only, no per-payer/per-account
// detail. Reuses computeApifyStats() above verbatim (same cache, same in-flight
// de-dupe, same Apify API calls) — this function only RESHAPES an already-computed
// object, it never talks to the Apify API itself.
//
// Explicit ALLOW-list (not an exclude-list): a new private field added above later
// defaults to hidden here unless someone deliberately adds it to one of the two
// lists below — the safer failure direction for a public, key-less route. This is
// also why `id` (the Apify actor id) and `stale_reason` (an internal error message,
// present on "stale"/"unavailable" freshness) are dropped: neither is in the lists,
// so neither is ever forwarded, without needing a dedicated "delete these" step.
const PUBLIC_TOP_LEVEL_FIELDS = [
  "generated_at",
  "freshness",
  "apify_last_fetched_at",
  "monetized_actors_count",
  "revenue_estimated_gross_usd",
  "revenue_estimated_net_usd",
  "succeeded_runs_30d",
  "runs_all_time",
...
const PUBLIC_ACTOR_FIELDS = [
  ...
  "succeeded_runs_30d",
  "runs_all_time",
  "estimated_gross_revenue_30d_usd",
  "estimated_gross_revenue_all_time_usd",
];

export async function computeApifyStatsPublic() {
  const full = await computeApifyStats();
  const scrubbed = {};
  for (const key of PUBLIC_TOP_LEVEL_FIELDS) {
    if (key in full) scrubbed[key] = full[key];
  }
  scrubbed.by_actor = (full.by_actor || []).map((actor) => {
    const publicActor = {};
    for (const key of PUBLIC_ACTOR_FIELDS) {
      if (key in actor) publicActor[key] = actor[key];
    }
    return publicActor;
  });
  return scrubbed;
