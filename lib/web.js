...
//
// Not exported: only used internally by fetchPageSafely below. It WAS
// briefly exported (2026-09-03) so the now-retired POST /api/web/scrape
// could reuse this exact mechanism instead of duplicating it — reverted to
// internal-only when that endpoint was removed (see lib/tavily.js's header
// comment for why). Re-export it again if a future endpoint needs the same
// robots.txt check outside lib/web.js.
